...
Architecture changes vs bbnet_aaa_v1.py:
  ...
  3. 88×88 bottleneck with 3 AAB (replaces 44×44 with 9 AAB) — 2× spatial precision
  4. Separate RGB/guidance stems + SAFM modulation (replaces 6ch concat)

Resolution ladder:
  Input [B,3+4, 1408] → stems stride-2 → [B,32,704] → SAFM fuse
  Encoder: [B,32,704] → stem s2 → [B,32,352] → Stage0(1) → Stage1(4)@176 → Stage2(3)@88
  AAA Bottleneck: 1 RG × 3 AAB at 88×88 with DynRelPos2d(f_stop)
...
class Inception_Encoder_Unet_Decoder(BaseModel):
    ...
    def forward(self, *inputs):
        ...
        # Pass through 1 Residual Group (coord + kernel conditioning at each)
